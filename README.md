...
- **Image generation**. Prompt editor with style presets, model/size/count/seed/negative-prompt controls, live progress, and a results gallery. Multiple images (1–6) run as parallel single-image requests, so counts work even on models like `qwen-image-3.0-pro` whose API ignores the batch parameter; a fixed seed is offset per image (seed, seed+1, …) so batches stay distinct and reproducible. A dice button fills in a random seed, and the watermark is off by default.
- **Multiple providers**. Images through Alibaba Bailian (via the `bl` CLI), MiniMax, Pollinations, Google Gemini, Cloudflare Workers AI, Hugging Face Inference Providers, Meta Muse Image, DeepInfra, SiliconFlow, or any OpenAI-compatible images endpoint; video through Bailian and MiniMax; music through MiniMax; and speech through MiniMax or Fish Audio. Switch providers per job or hide unused providers in Settings. MiniMax returns up to 9 images per request with fixed aspect ratios.
- **Keyless and account-backed options**. Pollinations needs no API key. Cloudflare needs an account ID and API token; Gemini, Hugging Face, Meta Muse, DeepInfra, and SiliconFlow each need their provider's API key; the OpenAI-Compatible provider needs a key plus an endpoint base URL. Pricing and free allowances depend on the provider and model.
...
| **Video** | Text-to-video and image-to-video. Bailian runs `bl video generate` (happyhorse / wan2.6 models, including `happyhorse-1.1-r2v` reference-to-video, which uses the image input as the reference frame) with resolution, aspect ratio, duration, seed, and watermark controls. MiniMax runs through the `mmx` CLI: MiniMax-H3 (Video Generation V2, 2K output, 4–15 s clips, aspect-ratio control) or Hailuo-2.3; image-to-video takes a local first frame or an image URL. Results play inline and are saved to your library folder. |
...
| **API Keys** | Store multiple API keys in the macOS Keychain, each tagged as Bailian, MiniMax, Google Gemini, Fish Audio, Cloudflare Workers AI, Hugging Face, Meta Muse Image, DeepInfra, SiliconFlow, or OpenAI-Compatible, and test them. Cloudflare keys also take an Account ID; Hugging Face keys take an inference provider name; OpenAI-Compatible keys take an endpoint base URL; Bailian keys can take a `--base-url` endpoint override (e.g. a Qwen Cloud token-plan endpoint). The first key added for each provider is used by default; pick a different one in Settings → Per-provider API key. Keys stored before provider tags existed are re-attributed automatically on first launch (via key prefixes and your local usage history). |
...
Each service uses the preferred key for its provider, falling back to the first stored key for that provider. Bailian image, chat, and vision calls pass the selected Bailian key to `bl` as `--api-key`. A Bailian key can also carry an endpoint override (`--base-url`), which is how you use a **Qwen Cloud Token Plan** key alongside a regular Bailian key: add both as Bailian keys, give the Token Plan key the base URL `https://token-plan.cn-beijing.maas.aliyuncs.com`, then pick per provider in Settings which one is active. With no override, the active `bl` profile decides.
...
- DeepInfra, SiliconFlow, and OpenAI-Compatible share one OpenAI-images client: `POST {base}/v1/images/generations` with a bearer key. DeepInfra (`api.deepinfra.com`) and custom endpoints use the standard schema (`size`, `n`, `response_format: b64_json`); SiliconFlow (`api.siliconflow.cn`) uses `image_size` / `batch_size`. Responses may be base64 or a URL; URLs are downloaded to the library folder. Custom endpoints store their base URL alongside the key.
...
- DeepInfra, SiliconFlow, and OpenAI-Compatible are pay-per-image. DeepInfra's Test button runs a real 256×256 FLUX-schnell probe (~$0.0003) because its model list is public and wouldn't otherwise prove the key; SiliconFlow and custom endpoints validate against the key-gated `/v1/models` listing. SiliconFlow's request schema differs (`image_size` / `batch_size`), which BlStudio maps for you. For the OpenAI-Compatible provider, model ids are free-typed — anything your endpoint exposes works.
